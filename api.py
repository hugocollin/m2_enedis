...
class API:
    ...
    def get_data(self):
        ...
        # Transformation des données en DataFrame avec types optimisés
        all_dpe = pd.DataFrame(all_dpe, dtype='str')

        # Mise en forme des données
        all_dpe.drop(columns=["_score"], inplace=True, errors='ignore')
        ...
        try:
            df = pd.read_csv(
                "assets/data_69.csv",
                sep="|",
                dtype={"Date réception DPE": str},
                low_memory=False
            )
            
            # Conversion en datetime pour comparer les dates
            df["Date réception DPE"] = pd.to_datetime(df["Date réception DPE"], format="%Y%m%d", errors='coerce')
            all_dpe["Date_réception_DPE"] = pd.to_datetime(all_dpe["Date_réception_DPE"], format="%Y%m%d", errors='coerce')
            
            # Récupération de la date des DPE les plus récents
            max_date = df["Date réception DPE"].max()
            
            # Filtrage des nouveaux DPE
            all_dpe = all_dpe[all_dpe["Date_réception_DPE"] > max_date]
            ...
            for i in range(0, len(dpe_list), 500):
                chunk = dpe_list[i:i + 500]
                ...
                try:
                    ...
                    # Transformation immédiate et écriture dans le CSV
                    chunk_data = pd.DataFrame(data["results"], dtype='str')
                    chunk_data.drop(columns=["_score"], inplace=True, errors='ignore')
                    chunk_data[["Latitude", "Longitude"]] = chunk_data["_geopoint"].str.split(",", expand=True)
                    chunk_data.drop(columns=["_geopoint"], inplace=True, errors='ignore')
                    chunk_data["Date_réception_DPE"] = chunk_data["Date_réception_DPE"].str.replace("-", "")
                    chunk_data["Date_réception_DPE_graph"] = chunk_data["Date_réception_DPE"].str[:6]
                    chunk_data.rename(columns={
                    ...
                    print(f"Chunk {i//500 + 1} traité et ajouté au fichier.")
                except Exception as e:
                    print(f"Erreur lors du traitement du chunk {i//500 + 1} : {e}")
                    break

            print(f"{len(dpe_list)} DPE ajoutés")
            
        except pd.errors.DtypeWarning:
            print("Avertissement de type de données lors de la lecture du CSV.")
            ...
            while True:
                try:
                    if page_number == 1:
                        ...
                        print(f"{total_rows} nouveaux DPE trouvés : récupération des données en cours, cela peut prendre un moment...")
                    else:
                    ...
                    print(f"Chunk {page_number} traité et ajouté au fichier.")
                    ...
                    print(f"Erreur lors du traitement de la page {page_number} : {e}")
                    break
